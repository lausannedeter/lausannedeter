...
<template>
  ...
  <section v-else class="org-page">
    ...
    <div class="hero">
      <div class="title-block">
        <div class="title-main">
          <h2 class="title">{{ $t("espaceOrgas.title") }}</h2>
        </div>
        <div class="title-sub"></div>
      </div>

      <p class="hero-description">
        {{ $t("espaceOrgas.paragraph.part1") }}
        <br /><br />
        <span style="white-space: nowrap;">
            {{ $t("espaceOrgas.paragraph.part2") }}<a class="deter">Lausanne Deter</a>{{ $t("espaceOrgas.paragraph.part3") }}
        </span>
      </p>
    </div>

    <div class="why-section">
      <h3 class="section-label">{{ $t("espaceOrgas.whyJoin.title") }}</h3>
      <div class="why-grid">
        <div class="why-card" :style="{ borderLeft: '3px solid #FFBF00', backgroundColor: '#FFF8E1' }">
          ...
          <p class="why-text">{{ $t("espaceOrgas.whyJoin.publish") }}</p>
        </div>
        <div class="why-card" :style="{ borderLeft: '3px solid #41CFA9', backgroundColor: '#E0F2F1' }">
          ...
          <p class="why-text">{{ $t("espaceOrgas.whyJoin.visibility") }}</p>
        </div>
        <div class="why-card" :style="{ borderLeft: '3px solid #FA41AE', backgroundColor: '#FDE4F2' }">
          ...
          <p class="why-text">{{ $t("espaceOrgas.whyJoin.solidarity") }}</p>
        </div>
      </div>
    </div>
    ...
    <div class="register-section">
      <h3 class="section-label">{{ $t("espaceOrgas.accessDemand.title") }}</h3>
      <p class="register-hint">
        {{ $t("espaceOrgas.accessDemand.paragraph") }}
      </p>

      <Transition name="fade">
        <div v-if="status === 'success'" class="banner banner--success">
          ✓ {{ $t("espaceOrgas.accessDemand.success") }}
        </div>
      </Transition>
      <Transition name="fade">
      ...
      <form class="register-form" @submit.prevent="handleRegister" novalidate>
        ...
        <div class="field">
          <label class="label" for="org-name">{{ $t("espaceOrgas.accessDemand.form.name.label") }} <span class="req">*</span></label>
          <input
            id="org-name"
            v-model="form.name"
            class="input"
        ...
        <div class="field">
          <label class="label" for="org-email">{{ $t("espaceOrgas.accessDemand.form.email.label") }} <span class="req">*</span></label>
          <input
            id="org-email"
            v-model="form.email"
            class="input"
        ...
        <div class="field">
          <label class="label" for="org-desc">{{ $t("espaceOrgas.accessDemand.form.presentation.label") }} <span class="req">*</span></label>
          <textarea
            id="org-desc"
            v-model="form.description"
            class="input textarea"
            rows="3"
            :placeholder="$t('espaceOrgas.accessDemand.form.presentation.placeholder')"
            required
            @input="status = null"
          ></textarea>
        </div>
        ...
        <div class="field">
          <label class="label" for="org-url">{{ $t("espaceOrgas.accessDemand.form.link.label") }}</label>
          <input
            id="org-url"
            v-model="form.url"
            class="input"
        ...
        <div class="form-footer">
          <span class="req-note"><span class="req">*</span> {{ $t("espaceOrgas.accessDemand.form.required") }}</span>
          <button
          ...
          >
            ...
            <span v-else>{{ $t("espaceOrgas.accessDemand.form.submit") }}</span>
          </button>
        </div>
      ...
      <p class="login-redirect">
        {{ $t("espaceOrgas.accessDemand.connect.label") }}
        <nuxt-link class="login-link" :to="localePath('/login')">{{ $t("espaceOrgas.accessDemand.connect.button") }} →</nuxt-link>
      </p>
    </div>
