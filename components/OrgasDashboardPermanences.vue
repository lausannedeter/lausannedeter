...
const permanencesGrouped = computed(() => props.events.reduce((acc, event) => {
    if (!acc[event.title]) {
        acc[event.title] = [];
    }
...
<template>
    <div class="events-section">
        ...
        <h3 class="section-label">Mes permanences</h3>
        ...
        <div v-else class="group-list">
            <div v-for="(group, title) in permanencesGrouped" :key="title" class="group-container">
                <div class="group-title-row" :style="{
                            borderLeft: `5px solid ${categoryMap[group[0].category].color ?? '#ccc'}`
                        }">
                    <div class="event-info">
                        <span class="event-title">{{ title }}</span>
                    </div>
                    <div class="event-actions">
                        <button class="icon-btn" @click="toggleGroup(title)">
                            <img class="btn-img" :class="{ isOpen: openGroupsMenu[title] }" src="/icons/arrow-down.png"
                                alt="arrow-down.png">
                        </button>
                    </div>
                </div>

                <div v-show="openGroupsMenu[title]" class="group-events-container">
                    <div v-for="event in group" :key="event._id" class="event-row"
                        ...
                        <div class="event-actions">
                            ...
                            <button class="icon-btn icon-btn--danger" :disabled="deleting === event._id"
                                title="Supprimer" @click="deleteEvent(event._id)">
